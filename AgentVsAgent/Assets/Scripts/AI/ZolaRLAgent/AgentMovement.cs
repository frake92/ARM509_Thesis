...
public class AgentMovement : MonoBehaviour
{
    ...
    public void Initialize(ZolaRLAgent agent, ZolaRLAgent opponent, Transform centerOfThing)
    {
        this.agent = agent;
    ...
    public void UpdateFacingDirection()
    {
        ...
        if (opponent.centerOfThing.transform.position.x <= centerOfThing.transform.position.x)
        {
    ...
    {
        if (opponent == null || agent.IsMovementDisabled) return;
        Vector3 dirToPlayer = (opponent.transform.position - transform.position).normalized;
        ...
        Vector3 idealPos = opponent.transform.position - dirToPlayer * targetDistance;
